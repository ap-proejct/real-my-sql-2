...
## 1. 문자열(CHAR와 VARCHAR)
### 1. 저장 공간
- CHAR과 VARCHAR의 공통점은 문자열을 저장할 수 있는 데이터타입이라는 점이고, 가장 큰 차이는 고정 길이냐 가변 길이냐 이다
...
- FLOAT는 4바이트를 사용해 유효 자릿수를 7개까지 표현할 수 있다
...
- MySQL에서 인덱스 레코드의 모든 칼럼의 최대 크기를 가지고 있는데 이 제한을 넘어서는 인덱스를 생성할 수 없다
  - utf8mb4는 768글자까지만 인덱스를 생성할 수 있다
...
- `COMPACT`포맷에서 저장할 수 있는 레코드의 최대 길이는 8126바이트이다
  - 레코드 전체 길이가 8126바이트를 넘는다면 용량이 큰 칼럼 순서대로 외부 페이지로 옮기면서 레코드 길이를 줄이려고 한다
  - 외부 페이지에 저장하고 각 페이지는 체인으로 연결한다
  - 외부 페이지로 저장하는 경우 앞쪽 768바이트(BLOB 프리픽스)만 잘라서 프라이머리 키 페이지에 같이 저장한다
  - 인덱스 생성에 도움이 되기는 하지만 저장 효율이 떨어지고 BLOB이나 TEXT 칼럼을 참조하지 않는 쿼리는 성능도 떨어진다 

<img src="./images/external_page.png" width=500>
...
- 용량이 큰 JSON 도큐먼트가 저장되면 16KB 단위로 여러 개의 데이터 페이지로 나뉘어 저장된다
